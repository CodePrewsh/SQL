...
VALUES 
(1, '2024-08-15 15:30:00'), -- Thandeka's order
(2, '2024-08-16 16:00:00'); -- Precious's order


INSERT INTO OrderDetails (OrderID, ProductId, Qty)
VALUES 
(1, 1, 2),  -- Thandeka ordered 2 Cokes
(1, 2, 1),  -- Thandeka ordered 1 Chips
(2, 3, 2),  -- Precious ordered 2 Pepsis
(2, 4, 1);  -- Precious ordered 1 Cookies
...
-- Checkout
INSERT INTO OrderHeader (User_ID, Orderdate) VALUES (1, CURRENT_TIMESTAMP);
DO $$ DECLARE
    last_order_id INT;
BEGIN
    SELECT currval(pg_get_serial_sequence('OrderHeader', 'OrderID')) INTO last_order_id;
...
CREATE OR REPLACE FUNCTION AddItemToCart(p_ProductId INT, p_Quantity INT) 
RETURNS VOID AS $$
BEGIN
...
END;
$$ LANGUAGE plpgsql;
SELECT * FROM RemoveItemFromCart(1);
SELECT * FROM Cart;

CREATE OR REPLACE FUNCTION RemoveItemFromCart(p_ProductId INT) 
RETURNS VOID AS $$
BEGIN
